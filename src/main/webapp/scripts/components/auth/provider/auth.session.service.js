...
angular.module('publisherApp')
    .factory('AuthServerProvider', function loginService($rootScope, $http, localStorageService, $window, $uibModal, Principal, $q) {
        return {
            /*ORIGIN :
            ...
            login: function() {
                return $http.jsonp('app/login?callback=JSON_CALLBACK')
                    .success(function (response, status) {
                        ...
                        $rootScope.modalOpened = $uibModal.open({ templateUrl: 'scripts/app/account/login/loginModal.html', controller: 'LoginModalController', backdrop: false });
                        return $q.reject();
                    });
            },
            logout: function() {

            ...
            <ul class="list-group">
              <li v-for="classification in context.classifications" :key="classification.id" class="list-group-item">
                <figure :class="classification.type">
                  <img v-if="context.publisher.context.reader.classificationDecorations.includes('ENCLOSURE') && classification.iconUrl" :src="classification.iconUrl"/>
                  <figcaption>
...
export default {
  ...
  methods: {
    ...
    initDatas (response) {
      ...
      if (this.classifications && this.classifications.length > 0) {
        this.classifications.forEach(classification => {
          var found = false
          for (let i = 0; i < this.pubContexts.length; i++) {
            if (this.pubContexts[i].id === classification.data.publisher.id) {
              if (!(classification.data.publisher.context.redactor.writingMode === 'STATIC' &&
                ...
                this.pubContexts[i].classifications.push(classification.data)
                found = true
              }
            }
          }
    ...
    // Formatage d'une date au format 'DD MM. YYYY, HH:mm:ss'
    formatDateTime (date) {
    ...
    formatDateDayMonthYear (date) {
      var result = DateUtils.convertToIntString(DateUtils.convertDateTimeFromServer(date), {
        year: 'numeric',
        month: 'long',
        day: 'numeric'
      }, store.getters.getLanguage)

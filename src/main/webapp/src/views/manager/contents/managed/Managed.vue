<template>
  ...
  <div
    class="modal fade"
    id="validateItemConfirmation"
    ref="invalidateItemConfirmation"
  >
    <div class="modal-dialog">
      <div class="modal-content">
        <form name="invalidateForm">
          ...
          <div class="modal-footer">
            ...
            <button
              type="button"
              class="btn btn-danger"
              @click="confirmInvalidate(item.id)"
            >
              <span class="far fa-check-circle"></span>&nbsp;<span>{{
                $t("entity.action.unvalidate")
              }}</span>
            </button>
          </div>
        </form>
  ...
  <div
    class="modal fade"
    id="inValidateItemConfirmation"
    ref="validateItemConfirmation"
  >
    <div class="modal-dialog">
      <div class="modal-content">
  ...
  <div id="tabs">
    ...
    <ul class="nav nav-tabs">
      <li
        v-for="state in itemStateForManager"
        :key="state.label"
        class="nav-item"
      >
        <a
    ...
    <div class="table-responsive table-responsive-to-cards">
      <div
        v-if="organizations !== null && organizations.length > 1"
        class="filter-area row"
      >
        <h5>{{ $t("manager.contents.filter.title") }}</h5>
        <div class="organization col-auto">
          <label class="control-label visually-hidden" for="organization">{{
        ...
        <tbody>
          ...
          >
            ...
            <td :data-label="$t('item.created')">
              <span :data-label="$t('item.beforeDate')">{{
                formatDate(item.createdDate)
              }}</span>
              <span :data-label="$t('item.beforeName')">{{
            ...
            <td
              class="text-center fixed-date-width"
              :data-label="$t('item.startDate')"
            >
              {{ formatDateSimple(item.startDate) }}
            </td>
            <td
              class="text-center fixed-date-width"
              :data-label="$t('item.endDate')"
            >
            ...
            >
              <span
                v-if="item.validatedDate !== null"
                :data-label="$t('item.beforeDate')"
                >{{ formatDate(item.validatedDate) }}</span
              >
              <span
                v-if="item.validatedBy !== null"
                :data-label="$t('item.beforeName')"
                >{{ item.validatedBy.displayName }}</span
              >
            </td>
            ...
            <td
              class="d-md-none d-lg-none d-xl-table-cell text-center"
              :data-label="$t('item.rssAllowed')"
            >
      ...
      <nav>
        <ul class="pagination">
          <li
            v-if="links.first"
            @click.prevent="loadPage(links.first)"
            class="page-item"
          >
          ...
          <li
            v-if="links.prev"
            @click.prevent="loadPage(links.prev)"
            class="page-item"
          >
            <a class="page-link" href="">&lt;</a>
          </li>
          ...
          <li
            v-if="links.next"
            @click.prevent="loadPage(links.next)"
            class="page-item"
          >
            <a class="page-link" href="">&gt;</a>
          </li>
          <li
            v-if="links.last"
            @click.prevent="loadPage(links.last)"
            class="page-item"
          >
            <a class="page-link" href="">&gt;&gt;</a>
          </li>
...
export default {
  ...
  methods: {
    ...
    loadAll() {
      this.itemState = this.$route.params.itemState
        ? this.getEnumKey(this.$route.params.itemState)
        : this.getEnumKey("PENDING");
      let query_params = {
        page: this.page,
        per_page: 20,
      ...
      if (
        CommonUtils.isDefined(this.organization_filter) &&

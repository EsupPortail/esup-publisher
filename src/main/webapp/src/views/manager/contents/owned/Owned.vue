<template>
  ...
  <div>
    <div class="modal fade" ref="deleteItemConfirmation">
    ...
    <div id="tabs">
      ...
      <div class="table-responsive table-responsive-to-cards">
            ...
            <tbody>
              <tr v-for="item in items" :key="item.id" :class="{highlight:item.highlight}">
                ...
                <td class="d-none" :data-label="$t('item.enclosure')">
                    <img v-if="item.enclosure" name="enclosure" id="enclosure" src="{{'item.enclosure'}}" class="img-responsive" />
                </td>
        ...
        <nav>
          <ul class="pagination">
            <li v-if="links.first !== null" @click.prevent="loadPage(links.first)" class="page-item"><a class="page-link" href="">&lt;&lt;</a></li>
            <li v-if="links.prev !== null && page != 1" @click.prevent="loadPage(links.prev)" class="page-item"><a class="page-link" href="">&lt;</a></li>
            ...
            <li v-if="links.last !== null && page < links.last" @click.prevent="loadPage(page + 1)" class="page-item"><a class="page-link" href="">{{page + 1}}</a></li>
            <li v-if="links.last !== null && page < links.last - 1" @click.prevent="loadPage(page + 2)" class="page-item"><a class="page-link" href="">{{page + 2}}</a></li>
            <li v-if="links.next !== null  && page != links.last" @click.prevent="loadPage(links.next)" class="page-item"><a class="page-link" href="">&gt;</a></li>
            <li v-if="links.last !== null" @click.prevent="loadPage(links.last)" class="page-item"><a class="page-link" href="">&gt;&gt;</a></li>
          </ul>
        </nav>
        <div class="legend">
...
export default {
  ...
  methods: {
    // Méthode permettant de récupérer la liste des objets ownedContent
    loadAll () {
      this.itemState = this.$route.params.itemState ? this.getEnumKey(this.$route.params.itemState) : this.getEnumKey('DRAFT')
      ItemService.query({ page: this.page, per_page: 10, owned: true, item_status: this.itemState }).then((response) => {
        if (response) {
    ...
    itemDetail (item) {
      this.$router.push({ name: 'ContentDetail', params: { id: item.id, itemState: this.getEnumName(this.itemState) } })
    },
    update (itemId) {

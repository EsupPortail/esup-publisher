  ...
  <div class="publish-publisher">
    ...
    <div class="form-group" v-if="publishers.length >= 1">
        ...
        <select class="form-select" id="reader" name="reader" v-model="selectedPublisher.context.reader" @change="selectReader(selectedPublisher.context.reader)"
                required>
            <option v-for="reader in readers" :key="reader.id" :value="reader">{{reader.displayName}}</option>
        </select>
    </div>
    ...
    <div class="form-group" v-if="selectedPublisher.context.reader && selectedPublisher.context.reader.id && redactors.length > 1">
        ...
        <select class="form-select" id="redactor" name="redactor" v-model="selectedPublisher.context.redactor" @change="selectRedactor(selectedPublisher.context.redactor)"
                required>
            <option v-for="redactor in redactors" :key="redactor.id" :value="redactor">{{redactor.displayName}}</option>
        </select>
    </div>
    ...
    <div class="form-group" v-if="selectedPublisher.context.redactor && selectedPublisher.context.redactor.id" >
        ...
        <select class="form-select" id="organization" name="organization" v-model="selectedPublisher.context.organization" @change="selectOrganization(selectedPublisher.context.organization)"
                required>
            <option v-for="organization in sortedOrganizations" :key="organization.id" :value="organization">{{organization.name}}</option>
        </select>
    </div>
    ...
    <div class="text-center">
        <div class="btn-group" role="group">
            ...
            <router-link to="classification" custom v-slot="{navigate}">
              <button type="button" :disabled="!isPublisherSelected" class="btn btn-default btn-outline-dark btn-nav" @click="validatePublisher();navigate()">
...
export default {
  ...
  computed: {
    isPublisherSelected () {
      return this.selectedPublisher.context.reader !== null && this.selectedPublisher.context.reader !== undefined && !CommonUtils.equals({}, this.selectedPublisher.context.reader) &&
        this.selectedPublisher.context.redactor !== null && this.selectedPublisher.context.redactor !== undefined && !CommonUtils.equals({}, this.selectedPublisher.context.redactor) &&
        this.selectedPublisher.context.organization !== null && this.selectedPublisher.context.organization !== undefined && !CommonUtils.equals({}, this.selectedPublisher.context.organization)
    },
    sortedOrganizations () {
  ...
  methods: {
    ...
    validatePublisher () {
      if (this.isPublisherSelected) {
        for (var i = 0, size = this.publishers.length; i < size; i++) {
          if (this.publishers[i].context.reader.id === this.selectedPublisher.context.reader.id &&
    ...
    toArray (object) {
      return CommonUtils.isArray(object) ? object : Object.keys(object).map(key => { return object[key] })
    }
  },
  created () {
    if (this.publisher === null || this.publisher === undefined || CommonUtils.equals({}, this.publisher)) {

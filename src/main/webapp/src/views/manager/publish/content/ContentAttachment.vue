<template>
  <div>
    <div class="attachmentForm">
      ...
      <div class="row">
        ...
        <div class="form-group col-md-4">
          <label class="control-label" for="endDate">{{
            $t("attachment.endDate")
          }}</label>
          <span
            v-if="publisher.context.redactor.optionalPublishTime"
            class="d-block d-md-none"
            >{{ $t("attachment.detail.unlimitedEndDate") }}</span
          >
          <input
            type="date"
            class="form-control"
      ...
      <div class="form-group">
        <label class="control-label" for="linkedFiles">{{
          $t("attachment.linkedFiles")
        }}</label>
        <div class="form-inline">
          <div
            v-for="attachment in filteredLinkedFilesToContent"
            :key="attachment.uri"
            class="linkedFiles"
          >
            <span>
        ...
        <div
          ...
          v-if="formValidator.hasError('files', formErrors.REQUIRED)"
        >
          {{ $t("entity.validation.required") }}
        </div>
        <div

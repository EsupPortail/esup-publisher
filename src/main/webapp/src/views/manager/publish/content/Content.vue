    ...
    <div class="form-group row" v-if="itemTypeList.length > 1">
      ...
      <div class="col-md-8" v-if="(item === null || item === undefined || !item.id) && itemTypeList.length > 1">
        <div class="form-check form-check-inline" v-for="itemType in itemTypeList" :key="itemType">
          <input class="form-check-input" type="radio" name="type" :id="'itemType'  + itemType" :value="itemType" v-model="content.type" @change="changeContentType('{{content.type}}')">
    ...
    <div class="text-center">
      <div class="btn-group" role="group">
        ...
        <router-link to="/home" custom v-slot="{navigate}">
          <button type="button" class="btn btn-default btn-outline-dark btn-nav" @click="cancel();navigate()">
    ...
    <div class="modal fade" id="deleteAttachmentConfirmation" ref="deleteAttachmentConfirmation">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>{{ $t("attachment.delete.question", { id: attachmentToDel ? attachmentToDel.filename : '' }) }}</p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default btn-outline-dark" data-bs-dismiss="modal">
                        <span class="fas fa-ban"></span>&nbsp;<span>{{ $t("entity.action.cancel") }}</span>
...
export default {
  ...
  provide () {
    return {
      ...
      setProgressStatus: (progressStatus) => { this.progressStatus = progressStatus },
  ...
  inject: [
    'contentData', 'publisher', 'setPublisher', 'item', 'setItem', 'highlight', 'linkedFilesToContent', 'setLinkedFilesToContent',
    'itemValidated', 'setItemValidated'
  ],
  computed: {
    itemStatusList () {
  ...
  methods: {
    initItem () {
      ...
      switch (this.content.type) {
        ...
        case 'ATTACHMENT':
          ...
          break
        default: throw new Error('Type not managed : ' + this.content.type)
      }
      this.setItem(item)
    },
    initCropper () {
    ...
    cancel () {
      ...
      if (!CommonUtils.isDefined(this.item.enclosure) || this.item.enclosure === null || this.item.enclosure === undefined || !CommonUtils.isDefined(this.item.id)) {
        return
      }
    ...
    confirmDeleteAttachment () {
      FileManagerService.delete(this.publisher.context.organization.id, false, Base64Utils.encode(this.attachmentToDel.uri)).then(() => {
        this.setLinkedFilesToContent(this.linkedFilesToContent.filter(val => {
    ...
    uploadLinkedFile (file, filename, isImage, isPublic, successCallBack) {
      ...
      this.$toast.warning(t('errors.upload.filesize', { size: CommonUtils.convertByteToDisplayedString(maxSize, 2) }))
      return true
    },

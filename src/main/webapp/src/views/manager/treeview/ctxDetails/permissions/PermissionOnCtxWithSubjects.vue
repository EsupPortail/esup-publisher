<template>
  <h3 class="mt-3 mb-2">{{$t('permissionOnClassificationWithSubjectList.detail.title')}} &nbsp;
    <span class="hand" @click="changePermissionAdvanced()" v-has-role="'ROLE_ADMIN'" v-tooltip="$t('permission.advancedMode')" :class="{disabled : !permissionAdvanced}"><i class="fa fa-graduation-cap" /></span>
  </h3>
  ...
  <div v-show="!addPerm" aria-labelledby="addPermissionOnContextWithSubjectListLabel" aria-hidden="true">
    <form name="editPermOnCtxForm" role="form">
      <div class="header">
          <h4 id="myPermissionOnContextLabel">{{$t('permissionOnClassificationWithSubjectList.home.createOrEditLabel')}}</h4>
      </div>
      <div class="body">
        ...
        <div class="form-group" v-show="!permissionAdvanced && !isAdvancedEvaluator(permission.evaluator)">
          ...
          <esup-subject-search-button .searchId="'permSubject'" .config="subjectSearchButtonConfig"
            .onSelection="updateSelectedSubject"></esup-subject-search-button>
      ...
      <div class="form-group">
        ...
        <esup-subject-search-button .searchId="'authorizedSubject'" .config="subjectSearchButtonConfig"
          .onSelection="updateAuthorizedSubjects"></esup-subject-search-button>
        <ul class="list-group list-unstyled">
            <li v-for="target in permission.authorizedSubjects" :key="target.keyId" class="list-group-item">
              <esup-subject-infos .subject="target" .config="subjectInfosConfig" .onSubjectClicked="() => viewSubject(subject)">
                <a @click.prevent="removePermTarget(target)" v-tooltip="$t('manager.publish.targets.remove')" href=""><i class="far fa-times-circle text-danger" ></i></a>&nbsp;
              </esup-subject-infos>
            </li>
        </ul>
        <div class="invalid-feedback d-block"
          v-if="!permission.authorizedSubjects || permission.authorizedSubjects.length < 1">
          {{ $t('entity.validation.required') }}
        </div>
      </div>
      ...
      <div class="footer">
        ...
        <button type="button" @click="createPermission()" :disabled="!permission.role || permissionAdvanced && editEvaluatorError || !permissionAdvanced && (!areSubjectsSelected || isAdvancedEvaluator(permission.evaluator)) || !permission.authorizedSubjects || permission.authorizedSubjects.length < 1" class="btn btn-primary">
        ...
        <table class="table table-striped">
          <thead>
            <tr>
              ...
              <th>{{$t('permissionOnClassificationWithSubjectList.authorizedSubjects')}}</th>
              <th class="action"></th>
            </tr>
          </thead>
          <tbody>
          ...
          <tbody>
            <tr v-for="permission in permissions" :key="permission">
              ...
              <td class="action">
                ...
                <button type="button" v-if="!isAdvancedEvaluator(permission.evaluator) && canEditCtxPerms" @click="deletePermission(permission.id)" class="btn btn-danger">
                  <span class="far fa-times-circle"></span>&nbsp;<span>{{$t('entity.action.delete')}}</span>
                </button></td>
            </tr>
          </tbody>
...
export default {
  ...
  inject: [
    'addPerm', 'selectedSubjects', 'permissions', 'permission', 'permissionAdvanced', 'availableRoles', 'isDatasLoad',
    'editEvaluatorError', 'setIsDatasLoad', 'context', 'userDisplayedAttrs', 'userFonctionalAttrs',
    'deletePermission', 'changePermissionAdvanced', 'updateSelectedSubject', 'removeFromSelectedSubjects', 'areSubjectsSelected',
    'clearPermission', 'createPermission', 'addPermission', 'getPermissionTypeLabel', 'updatePermission', 'onModificationEditEvaluator',
    'isAdvancedEvaluator', 'subjectInfosConfig', 'selectPermissionManager', 'operatorTypeList', 'stringEvaluationModeList',
    'updateAuthorizedSubjects', 'removePermTarget'
  ],
  methods: {
    initEvaluator () {

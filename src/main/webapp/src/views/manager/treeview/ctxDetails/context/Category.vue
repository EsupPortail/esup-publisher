<template>
  <div>
    ...
    <div class="mb-3">
      ...
      <button type="button"
        @click="deleteCategory()" v-can-delete="context.contextKey"
    ...
    <dl class="row entity-details">
      <dt class="col-sm-5"><span>{{$t('category.name')}}</span></dt>
      <dd class="col-sm-7"><span>{{context.name}}</span></dd>
    </dl>
    <dl class="row entity-details">
      <dt class="col-sm-5"><span>{{$t('category.description')}}</span></dt>
      <dd class="col-sm-7"><span>{{context.description}}</span></dd>
    </dl>
    <dl class="row entity-details" v-if="context.iconUrl && context.iconUrl !== '' && context.iconUrl !== 'http://' && context.publisher && context.publisher.context.reader.classificationDecorations.includes('ENCLOSURE')">
      <dt class="col-sm-5"><span>{{$t('category.iconUrl')}}</span></dt>
      <dd class="col-sm-7"><img class="media-object img-fluid" :src="context.iconUrl" alt="image" /></dd>
    </dl>
    <dl class="row entity-details" v-if="context.publisher && context.publisher.context.reader.classificationDecorations.includes('COLOR')">
      <dt class="col-sm-5"><span>{{$t('category.color')}}</span></dt>
      <dd class="col-sm-7">
    ...
    <dl class="row entity-details" v-has-role="'ROLE_ADMIN'">
      <dt class="col-sm-5"><span>{{$t('category.accessView')}}</span></dt>
      <dd class="col-sm-7"><span>{{$t(getAccessTypeLabel(context.accessView))}}</span></dd>
    </dl>
    <dl class="row entity-details" v-has-role="'ROLE_ADMIN'">
      <dt class="col-sm-5"><span>{{$t('category.rssAllowed')}}</span></dt>
      <dd class="col-sm-7"><span><input type="checkbox" class="input-sm" v-model="context.rssAllowed" disabled></span></dd>
    </dl>
    <dl class="row entity-details">
      <dt class="col-sm-5"><span>{{$t('category.defaultDisplayOrder')}}</span></dt>
      <dd class="col-sm-7"><span>{{$t(getDisplayOrderTypeLabel(context.defaultDisplayOrder))}}</span></dd>
    </dl>
    <dl class="row entity-details" v-has-role="'ROLE_ADMIN'">
      <dt class="col-sm-5"><span>{{$t('category.displayOrder')}}</span></dt>
      <dd class="col-sm-7"><span>{{context.displayOrder}}</span></dd>
    </dl>
    <dl class="row entity-details" v-has-role="'ROLE_ADMIN'">
      <dt class="col-sm-5"><span>{{$t('category.lang')}}</span></dt>
      <dd class="col-sm-7"><span>{{$t(getLangLabel(context.lang))}}</span></dd>
    </dl>
    <dl class="row entity-details" v-has-role="'ROLE_ADMIN'">
      <dt class="col-sm-5"><span>{{$t('category.ttl')}}</span></dt>
      <dd class="col-sm-7"><span>{{context.ttl}}</span></dd>
    </dl>
    <dl class="row entity-details">
      <dt class="col-sm-5"><i class="fa fa-rss fa-lg"></i></dt>
      <dd class="col-sm-7"><a :href="appUrl + 'feed/rss/' + (context.publisher ? context.publisher.context.organization.id : '') + '?pid=' + (context.publisher ? context.publisher.id : '') + '&cid=' + context.id" target="_blank">
      {{appUrl}}feed/rss/{{context.publisher ? context.publisher.context.organization.id : ''}}?pid={{context.publisher ? context.publisher.id : ''}}&cid={{context.id}}</a></dd>
    </dl>

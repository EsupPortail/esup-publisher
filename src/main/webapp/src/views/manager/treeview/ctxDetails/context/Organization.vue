  ...
  <div>
    ...
    <div class="mb-3">
      ...
      <button type="button" @click="deleteOrganization()"
        class="btn btn-danger btn-sm me-1" v-has-role="'ROLE_ADMIN'">
        <span class="far fa-times-circle"></span>&nbsp;<span>{{$t("entity.action.delete")}}</span>
      </button>
    </div>

    <dl class="row entity-details">
      <dt class="col-sm-5"><span>{{$t('organization.name')}}</span></dt>
      <dd class="col-sm-7"><span>{{context.name}}</span></dd>
    </dl>
    <dl class="row entity-details">
      <dt class="col-sm-5"><span>{{$t('organization.description')}}</span></dt>
      <dd class="col-sm-7"><span>{{context.description}}</span></dd>
    </dl>
    <dl class="row entity-details" v-has-role="'ROLE_ADMIN'">
      <dt class="col-sm-5"><span>{{$t('organization.displayOrder')}}</span></dt>
      <dd class="col-sm-7"><span>{{context.displayOrder}}</span></dd>
    </dl>
    <dl class="row entity-details" v-has-role="'ROLE_ADMIN'">
      <dt class="col-sm-5"><span>{{$t('organization.allowNotifications')}}</span></dt>
      <dd class="col-sm-7"><span><input type="checkbox" v-model="context.allowNotifications" disabled/></span></dd>
    </dl>
    <dl class="row entity-details">
      <dt class="col-sm-5"><i class="fa fa-rss fa-lg"></i></dt>
      <dd class="col-sm-7"><a :href="appUrl + 'feed/rss/' + context.id" target="_blank">{{appUrl}}feed/rss/{{context.id}}</a></dd>
    </dl>
        ...
        <div class="modal-content">
          <form name="editForm" role="form" class="was-validated" novalidate show-validation>
            <div class="modal-header">
              <h4 class="modal-title" id="myOrganizationLabel">{{$t('organization.home.editLabel')}}</h4>
            ...
            <div class="modal-body">
              ...
              <div class="form-group">
                <label for ="name" class="control-label">{{$t('organization.name')}}</label>
                  <input type="text" class="form-control" name="name" id="name"
                        v-model="editedContextName" required minlength="5" maxlength="255" :disabled="!isInRole('ROLE_ADMIN')">
              ...
              <div class="form-group">
                ...
                <input type="text" class="form-control" name="description" id="description"
                        v-model="editedContextDescription" required minlength="5" maxlength="512" :disabled="!isInRole('ROLE_ADMIN')">
                <div class="invalid-feedback"
              ...
              <div class="form-group" v-has-role="'ROLE_ADMIN'">
                ...
                <input type="text" id="identifiers" name="identifiers" class="form-control" v-model="editedContextIdentifiers"
                        :placeholder="$t('organization.identifiers-help')" required :disabled="!isInRole('ROLE_ADMIN')">
                <div class="invalid-feedback"
              ...
              <div class="form-group" v-has-role="'ROLE_ADMIN'">
                <label for="displayOrder" class="control-label">{{$t('organization.displayOrder')}}</label>
                  ...
                  v-model="editedContextDisplayOrder" required min="0" max="999" :disabled="!isInRole('ROLE_ADMIN')">
                <div class="invalid-feedback"
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <form name="deleteForm">
            ...
            <div class="modal-body">
                <p>{{$t('organization.delete.question', {id: context.name})}}</p>
            </div>
            <div class="modal-footer">
              <button type="button" class="btn btn-default btn-outline-dark" data-bs-dismiss="modal">
                  <span class="fas fa-ban"></span>&nbsp;<span>{{$t('entity.action.cancel')}}</span>

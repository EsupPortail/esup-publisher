<template>
  <div class="modal fade" id="saveCategoryModal" tabindex="-1" role="dialog" aria-labelledby="myCategoryLabel"
    aria-hidden="true" ref="saveCategoryModal">
    <div class="modal-dialog modal-fullscreen-md-down modal-lg">
      <div class="modal-content">
        <form name="editForm" role="form" class="was-validated" novalidate show-validation>
          <div class="modal-header">
            <h4 class="modal-title" id="myCategoryLabel">{{$t('category.home.createOrEditLabel')}}</h4>
          ...
          <div class="modal-body">
            <div class="form-group" v-has-role="'ROLE_ADMIN'">
              ...
              <input type="text" class="form-control" name="id" id="ID"
                v-model="editedContext.id" disabled>
            </div>
            <div class="form-group">
              <label for ="name" class="control-label">{{$t('category.name')}}</label>
              <input type="text" class="form-control" name="name" id="name"
                v-model="editedContextName" required minlength="3" maxlength="200">
            ...
            <div class="form-group">
              ...
              <input type="text" class="form-control" name="description" id="description"
                v-model="editedContextDescription" required minlength="5" maxlength="512">
              <div class="invalid-feedback"
            ...
            <div class="form-group" v-if="publisher.context.reader.classificationDecorations && publisher.context.reader.classificationDecorations.includes('COLOR')">
              <label for="color" class="control-label">{{$t('category.color')}}</label>
              <esup-color-palette-picker id="color" .color="editedContext.color" .config="colorPickerConfig" .onColorChanged="onColorChanged"></esup-color-palette-picker>
            </div>
            <div class="form-group" v-has-role="'ROLE_ADMIN'">
            ...
            <div class="form-group" v-has-role="'ROLE_ADMIN'">
              ...
              <input type="number" class="form-control" name="ttl" id="ttl"
                v-model="editedContextTtl" required min="900" max="86400">
              <div class="invalid-feedback"
            ...
            <div class="form-group" v-has-role="'ROLE_ADMIN'">
              ...
              <input type="text" class="form-control" name="publisher" id="publisher"
                :value="editedContext && editedContext.publisher ? editedContext.publisher.context.reader.displayName + '-' + editedContext.publisher.context.redactor.displayName : ''" disabled>
            </div>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-default btn-outline-dark" data-bs-dismiss="modal">
              <span class="fas fa-ban"></span>&nbsp;<span>{{$t('entity.action.cancel')}}</span></button>
            <button type="button" class="btn btn-primary" :disabled="formValidator.hasError() || invalidIconUrl || !canManage" @click="confirmUpdate" >
...
export default {
  ...
  inject: ['publisher', 'editedContext', 'setEditedContext', 'confirmUpdateContext', 'canManage', 'paletteColorPicker', 'autorizedDisplayOrderTypeList', 'langList'],
  computed: {
    editedContextName: {
      get () {

          ...
          <div class="modal-body">
            ...
            <template v-for="attribute in attributes" :key="attribute">
              <dl class="row entity-details">
                ...
                <dd class="col-sm-7">
                  <span
                    v-for="value in subject.attributes[attribute]"
                    :key="value.id"
                    class="list-comma"
                    >{{ value }}</span
                  >
                  <span v-if="!subject.attributes[attribute]" class="list-comma"
                    >&nbsp;</span
                  >
                </dd>
              </dl>
...
export default {
  ...
  methods: {
    ...
    showSubjectModal(target) {
      SubjectService.getSubjectInfos(target.keyType, target.keyId).then(
        (response) => {
          this.subject = response.data;
          this.subject.attributes = this.subject.attributes || [];
          this.attributes =
            target.keyType === "PERSON"
              ? SubjectService.getUserDisplayedAttrs()
              : SubjectService.getGroupDisplayedAttrs();

<template>
  <div>
    <h2><span>{{$t('reader.detail.title')}}</span> {{$route.params.id}}</h2>
        ...
        <table class="table table-striped">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" class="input-sm form-control" v-for="type in reader.authorizedTypes" :key="type" :value="$t('enum.itemType.' + type)" readonly>
                    </td>
                </tr>
                <tr>
                    <td>
                    ...
                    <td>
                        <input type="text" class="input-sm form-control" v-for="type in reader.classificationDecorations" :key="type" :value="$t('enum.classificationDecorType.' + type)" readonly>
                    </td>
                </tr>
            </tbody>
        </table>
...
export default {
  ...
  methods: {
    ...
    initData () {
      ReaderService.get(this.$route.params.id).then(result => {
        this.reader = result
      }).catch(error => {
        console.error(error)
      })

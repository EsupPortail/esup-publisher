...
<div>
  ...
  {{ $t("configuration.filter") }} <input type="text" v-model="filter" class="form-control">

  <table class="table table-sm table-striped table-bordered table-responsive" style="table-layout:fixed">
    <thead>
      <tr>
        <th @click="setSorting('prefix')" >{{ $t("configuration.table.prefix") }}</th>
      ...
      <tr v-for="entry in filteredConfiguration" :key="entry.prefix">
        ...
        <td>
          <div class="row" v-for="(value, key) in entry.properties" :key="key">
              ...
              <div class="col-md-6"><span class="float-end badge bg-info" style="white-space: normal;word-break:break-all">{{value}}</span></div>
          </div>
        </td>
      </tr>
    </tbody>
  ...
  computed: {
    filteredConfiguration () {
      ...
      if (this.filter !== null && this.filter !== '') {
        filterConfiguration = filterConfiguration.filter(conf => conf.prefix.includes(this.filter) || JSON.stringify(conf.properties).includes(this.filter))
      }

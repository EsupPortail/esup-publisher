  ...
  <div class="container-fluid justify-content-end">
    <div class="navbar-brand">
      ...
      <a class="navbar-brand"
        ...
        <span class="navbar-version">v{{ backVersion }}</span></a
      >
    </div>
          ...
          <ul class="dropdown-menu">
            <li :class="{ active: isPageNameIncludes('organization') }">
              <router-link to="/organization" class="dropdown-item"
                ><span class="fas fa-asterisk"></span> &#xA0;<span>{{
                  $t("global.menu.entities.organization")
                }}</span></router-link
              >
            </li>
            ...
            <li :class="{ active: isPageNameIncludes('redactor') }">
              <router-link to="/redactor" class="dropdown-item"
                ><span class="fas fa-asterisk"></span> &#xA0;<span>{{
                  $t("global.menu.entities.redactor")
                }}</span></router-link
              >
            </li>
            <li :class="{ active: isPageNameIncludes('publisher') }">
              <router-link to="/publisher" class="dropdown-item"
                ><span class="fas fa-asterisk"></span> &#xA0;<span>{{
                  $t("global.menu.entities.publisher")
                }}</span></router-link
              >
            </li>
          </ul>
          ...
          <ul class="dropdown-menu">
            ...
            <li :class="{ active: isPageNameIncludes('audits') }">
              <router-link to="/audits" class="dropdown-item"
                ><span class="fas fa-bell"></span> &#xA0;<span>{{
            ...
            <li :class="{ active: isPageNameIncludes('logs') }">
              <router-link to="/logs" class="dropdown-item"
                ><span class="fas fa-tasks"></span> &#xA0;<span>{{
            ...
            <li
              :class="{ active: isPageNameIncludes('console') }"
              v-if="environment === 'development'"
            >
              <a target="_blank" :href="`${dbConsoleUrl}`" class="dropdown-item"
                ><span class="fas fa-hdd"></span> &#xA0;<span>{{
                  $t("global.menu.admin.database")
                }}</span></a
              >
            </li>
          </ul>
        ...
        <li class="nav-item dropdown">
          ...
          >
            <span>
              ...
              <span class="hidden-tablet">{{
                $t("global.menu.language")
              }}</span>
            </span>
          </a>
          <ul class="dropdown-menu">
            <li
              v-active-menu="language"
              v-for="language in languages"
              :key="language"
              @click.prevent="switchLanguage(language)"
            >

...
class CommonUtils {
  ...
  isFile = function (file) {
    return (
      file != null &&
      (file instanceof Blob || (file.flashId && file.name && file.size))
    );
  };
  ...
  convertByteToDisplayedString(bytes, decimal) {
    if (
      ...
      var i = 0;
      while (
        i < byteSizeCompared.length - 1 &&
        bytes >= byteSizeCompared[i].val
      )
        i++;
      bytes /= i > 0 ? byteSizeCompared[i - 1].val : 1;
      return (
        this.convertToDecimal(bytes, decimal) + " " + byteSizeCompared[i].str
      );
    }
    return "NaN";
  }

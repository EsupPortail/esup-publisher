...
class ItemService {
  ...
  get(id) {
    return FetchWrapper.getJson("api/items/" + id).then((response) => {
      if (response.data) {
        response.data.endDate = DateUtils.convertLocalDateFromServer(
          response.data.endDate
        );
        response.data.startDate = DateUtils.convertLocalDateFromServer(
          response.data.startDate
        );
        response.data.validatedDate = DateUtils.convertDateTimeFromServer(
          response.data.validatedDate
        );

...
public class ObjectExistsException extends RuntimeException {
	...
	private static final long serialVersionUID = 3493558871904186534L;
	 ...
	 *
	 * @param name
	 *            the object's type name, usually a fully-qualified class name
	 * @param lookupFields
	 *            Names and values of the criteria that were used to look up the
	 *            object. Typically <code>{"id" => "foo"}</code>
	 * @return A user-readable object found message string.
	 */
	public static String message(final String name,
			final Map<String, ? extends Serializable> lookupFields) {

		final StringBuilder sb = new StringBuilder("Found existing {").append(
				name == null ? "OBJECT OF UNKNOWN TYPE" : name).append(
				"}. Lookup fields ");
		if (lookupFields == null || lookupFields.isEmpty()) {
			sb.append("{UNKNOWN}.");
		} else {
			for (Map.Entry<String, ? extends Serializable> lookupField : lookupFields
					.entrySet()) {
	...
	public void setLookupFields(Map<String, ? extends Serializable> lookupFields) { // NOPMD
																					// for
																					// serialization
		this.lookupFields = lookupFields;
	}

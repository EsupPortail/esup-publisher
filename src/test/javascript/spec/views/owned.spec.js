...
describe('Owned.vue tests', () => {
  ...
  it('test 1 Owned - Affichage d\'un élément dans la liste des publications', done => {
    ...
    ItemService.query = jest.fn().mockReturnValue(Promise.resolve({
      data: Object.assign([], [item1]),
      headers: {
        get: () => 'link'
      }
    }))
    ClassificationService.highlighted = jest.fn().mockReturnValue(Promise.resolve({
      data: { name: 'À la une' }
    }))
    EnumDatasService.getItemStatusList = jest.fn().mockReturnValue(
      [{ id: 0, name: 'PENDING', label: 'enum.itemStatus.pending.title' }, { id: 4, name: 'DRAFT', label: 'enum.itemStatus.draft.title' }]
    )
    ParseLinkUtils.parse = jest.fn().mockReturnValue({ last: 1, first: 1 })
    DateUtils.convertToIntString = jest.fn().mockReturnValue('1 janv. 2022, 12:00:00')
  ...
  it('test 3 Owned - itemDetail', done => {
    ...
    wrapper.vm.$nextTick(() => {
      ...
      wrapper.vm.$nextTick(() => {
        ...
        expect($router.push).toHaveBeenCalledWith({ name: 'ContentDetail', params: { id: item1.id } })
      })
      expect(EnumDatasService.getItemStatusList).toHaveBeenCalledTimes(1)
      expect(ItemService.query).toHaveBeenCalledTimes(1)
      expect(ClassificationService.highlighted).toHaveBeenCalledTimes(1)
  ...
  it('test 4 Owned - update', done => {
    ...
    wrapper.vm.$nextTick(() => {
      ...
      wrapper.vm.$nextTick(() => {
        ...
        expect($router.push).toHaveBeenCalledTimes(1)
        expect($router.push).toHaveBeenCalledWith({ name: 'PublishPublisher', params: { id: item1.id } })
      })
      expect(EnumDatasService.getItemStatusList).toHaveBeenCalledTimes(1)
      expect(ItemService.query).toHaveBeenCalledTimes(1)
      expect(ClassificationService.highlighted).toHaveBeenCalledTimes(1)
  ...
  it('test 5 Owned - deleteItem', done => {
    ...
    ItemService.get = jest.fn().mockReturnValue(Promise.resolve({ data: item1 }))

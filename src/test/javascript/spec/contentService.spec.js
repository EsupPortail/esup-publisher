...
describe('ContentService.js tests', () => {
  ...
  it('test 2 ContentService - get', (done) => {
    ...
    ContentService.get(id).then(value => {
      ...
      expect(value.data.item.body).toStrictEqual('<a href="' + process.env.VUE_APP_BACK_BASE_URL + 'link/to/file">test</a>')
      done()
    })
  })
  ...
  it('test 3 ContentService - update', (done) => {
    ...
    const data = {
      item: {
        ...
        body: '<a href="' + process.env.VUE_APP_BACK_BASE_URL + 'link/to/file">test</a>'
      },
      linkedFiles: [
        {
          uri: 'link/to/file'
    ...
    ContentService.update(data).then(() => {
      ...
      expect(FetchWrapper.putJson).toHaveBeenCalledWith('api/contents', expect.objectContaining({
  ...
  it('test 4 ContentService - save', (done) => {
    ...
    ContentService.save(data).then(() => {
      ...
      expect(FetchWrapper.postJson).toHaveBeenCalledWith('api/contents', expect.objectContaining({
        item: expect.objectContaining({
          endDate: '2021-12-01',
          startDate: '2021-12-02',
          body: '<a href="link/to/file">test</a>'

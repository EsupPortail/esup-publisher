import {
  FormValidationUtils,
  FormErrorType,
} from "@/services/util/FormValidationUtils.js";
...
describe("FormValidationUtils.js tests", () => {
  it("test 1 FormValidationUtils - TextField", () => {
    ...
    expect(formValidator.hasError(field, FormErrorType.REQUIRED)).toStrictEqual(
      true
    );
    ...
    formValidator.checkTextFieldValidity(field, value, min, max, required);
    expect(
      formValidator.hasError(field, FormErrorType.MIN_LENGTH)
    ).toStrictEqual(true);

    value = "testtesttest";
    formValidator.checkTextFieldValidity(field, value, min, max, required);
    expect(
      formValidator.hasError(field, FormErrorType.MAX_LENGTH)
    ).toStrictEqual(true);
  ...
  it("test 2 FormValidationUtils - NumberField", () => {
    ...
    formValidator.checkNumberFieldValidity(field, value, min, max, required);
    expect(
      formValidator.hasError(field, FormErrorType.MIN_VALUE)
    ).toStrictEqual(true);

    value = 11;
    formValidator.checkNumberFieldValidity(field, value, min, max, required);
    expect(
      formValidator.hasError(field, FormErrorType.MAX_VALUE)
    ).toStrictEqual(true);
  ...
  it("test 3 FormValidationUtils - DateField", () => {
    ...
    formValidator.checkDateFieldValidity(field, value, min, max, required);
    expect(formValidator.hasError(field, FormErrorType.MIN_DATE)).toStrictEqual(
      true
    );

    value = new Date(2022, 1, 15);
    formValidator.checkDateFieldValidity(field, value, min, max, required);
    expect(formValidator.hasError(field, FormErrorType.MAX_DATE)).toStrictEqual(
      true
    );
  ...
  it("test 4 FormValidationUtils - FileField", () => {
    ...
    formValidator.checkFileFieldValidity(field, value, pattern, size, required);
    expect(formValidator.hasError(field, FormErrorType.PATTERN)).toStrictEqual(
      true
    );

    value = { type: "image/jpeg", size: 600 };
    formValidator.checkFileFieldValidity(field, value, pattern, size, required);
    expect(formValidator.hasError(field, FormErrorType.MAX_SIZE)).toStrictEqual(
      true
    );
